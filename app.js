...
: `${count} word${count > 1 ? 's' : ''} you have got wrong`;
}
...
t.classList.toggle(‘active’, [‘practice’, ‘words’][i] === tab));
...
document.getElementById(‘mode-label’).textContent = currentMode === ‘mixed’
? ’🎲 Mixed - ’ + modeLabels[questionType]
: (modeLabels[currentMode] || modeLabels[questionType]);
...
if (!isArticle) setTimeout(() => document.getElementById(‘answer-input’).focus(), 50);
...
isCorrect = variants.some(v =>
userN === v || userN.replace(/^to /, ‘’) === v.replace(/^to /, ‘’));
} else {
...
document.getElementById(‘practice-card’).classList.add(‘flash-correct’);
...
if (questionType === ‘de-en’)      speakText(currentQuestion.german, ‘de-DE’);
else if (questionType === ‘en-de’) speakText(currentQuestion.english.replace(/^to /, ‘’), ‘en-US’);
else                               speakText(currentQuestion.german, ‘de-DE’);
}
...
const germanFinal = type === ‘noun’
? german.charAt(0).toUpperCase() + german.slice(1)
: german.toLowerCase();

if (words.some(w => normalizeWord(w.german) === normalizeWord(germanFinal))) {
...
spacer.style.height = (words.length * ROW_HEIGHT) + ‘px’;
